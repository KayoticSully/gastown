...
func TestEmitEvent(t *testing.T) {
	t.Run("basic event creation", func(t *testing.T) {
		...
		path, err := channelevents.EmitToTown(townRoot, "gastown", "test-channel", "MERGE_READY", []string{"polecat=nux", "branch=feat/test"})
		if err != nil {
	...
	t.Run("empty payload", func(t *testing.T) {
		townRoot := t.TempDir()
		path, err := channelevents.EmitToTown(townRoot, "gastown", "test-channel", "PATROL_WAKE", nil)
		if err != nil {
			t.Fatalf("EmitEvent failed: %v", err)
		}
	...
	t.Run("multiple events unique paths", func(t *testing.T) {
		...
		for i := 0; i < 5; i++ {
			path, err := channelevents.EmitToTown(townRoot, "gastown", "test-channel", "TEST", nil)
			if err != nil {
	...
	t.Run("malformed payload pair ignored", func(t *testing.T) {
		townRoot := t.TempDir()
		path, err := channelevents.EmitToTown(townRoot, "gastown", "test-channel", "TEST", []string{"valid=yes", "no-equals-sign"})
		if err != nil {
			t.Fatalf("EmitEvent failed: %v", err)
		}
...
func TestEmitEventChannelValidation(t *testing.T) {
	...
	_, err := channelevents.EmitToTown(townRoot, "gastown", "valid-channel", "TEST", nil)
	if err != nil {
		t.Errorf("valid channel name rejected: %v", err)
	}

	// Path traversal should be rejected
	_, err = channelevents.EmitToTown(townRoot, "gastown", "../etc", "TEST", nil)
	if err == nil {
		t.Error("expected error for path traversal channel name, got nil")
	}

	// Slash in channel should be rejected
	_, err = channelevents.EmitToTown(townRoot, "gastown", "foo/bar", "TEST", nil)
	if err == nil {
		t.Error("expected error for channel with slash, got nil")
	}

	// Empty channel should be rejected
	_, err = channelevents.EmitToTown(townRoot, "gastown", "", "TEST", nil)
	if err == nil {
