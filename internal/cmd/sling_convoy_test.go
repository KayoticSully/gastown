...
func TestConvoyTracksBeadExternalRef(t *testing.T) {
	...
	bdScript := `#!/bin/sh
echo '[{"dep_target":"external:gt-abc:gt-abc123"}]'
`
	bdPath := filepath.Join(binDir, "bd")
	if err := os.WriteFile(bdPath, []byte(bdScript), 0755); err != nil {
		t.Fatalf("write bd stub: %v", err)
...
func TestConvoyTracksBeadMultipleDeps(t *testing.T) {
	...
	bdScript := `#!/bin/sh
echo '[{"dep_target":"gt-other1"},{"dep_target":"external:gt-abc:gt-abc123"},{"dep_target":"gt-other2"}]'
`
	bdPath := filepath.Join(binDir, "bd")
	if err := os.WriteFile(bdPath, []byte(bdScript), 0755); err != nil {
		t.Fatalf("write bd stub: %v", err)
