...
func (d *testDAG) BdStubScript() string {
	...
	// bdDepListRawIDs calls (gt-c7j split schema):
	//   down: bd sql "SELECT COALESCE(depends_on_issue_id, depends_on_wisp_id, depends_on_external) AS dep_target FROM dependencies WHERE issue_id = '<id>' AND type = 'tracks'" --json
	//   up:   bd sql "SELECT issue_id FROM dependencies WHERE (depends_on_issue_id = '<id>' OR depends_on_wisp_id = '<id>' OR depends_on_external = '<id>') AND type = 'tracks'" --json
	sb.WriteString("  sql\\ *)\n")
	...
	// For "up" direction (bead → tracking convoys): match on depends_on_external = '<beadID>'
	// (the split-schema up query ORs all three target columns; matching the
	// external column uniquely identifies an up query — see gt-c7j).
...
// COALESCE(depends_on_issue_id, depends_on_wisp_id, depends_on_external) AS
// dep_target FROM dependencies WHERE issue_id = '<convoyID>' AND type = 'tracks'" --json`.
...
// FROM dependencies WHERE (depends_on_issue_id = '<beadID>' OR depends_on_wisp_id = '<beadID>'
// OR depends_on_external = '<beadID>') AND type = 'tracks'" --json`.
